...
# ==================================================
    ...
    if modo in [
        ...
        # SOLO AGREGADO
        costo_fijo_usd = st.number_input("Costo fijo mensual (USD)", value=0.0, step=100.0)
        tipo_cambio_usd = st.number_input("Tipo de cambio USD → PEN", value=3.80, step=0.01)
        ...
        if "tx_reference" in df.columns and "tx_amount" in df.columns:
            ...
            # SOLO AGREGADO
            total_operaciones = len(pagos)

            if total_operaciones > 0:
                costo_fijo_pen = costo_fijo_usd * tipo_cambio_usd
                costo_por_operacion = costo_fijo_pen / total_operaciones
            else:
