...
st.divider()
st.subheader("Descargar bases formato detalle")

# ===============================
# PREPARAR COLUMNAS BASE
# ===============================
base_detalle = comisiones.copy()

# FECHA (si existe en tu data)
if "fecha" in df.columns:
    base_detalle["FECHA"] = df["fecha"]
else:
    base_detalle["FECHA"] = ""

# SET_referencia (si existe)
if "set_referencia" in df.columns:
    base_detalle["SET_referencia"] = df["set_referencia"]
else:
    base_detalle["SET_referencia"] = ""

# CODIGO UNICO (deuda external)
if "deuda_external" in df.columns:
    base_detalle["CODIGO UNICO"] = df["deuda_external"]
else:
    base_detalle["CODIGO UNICO"] = ""

base_detalle["PY_CODIGO"] = pagos["tx_reference"].values
base_detalle["SF_CODIGO"] = fees["tx_reference"].values[:len(base_detalle)]

base_detalle["MONEDA"] = df["tx_currency_code"].values[:len(base_detalle)]
base_detalle["RECAUDO"] = base_detalle["tx_amount_pago"].round(2)
base_detalle["COMISION"] = base_detalle["comision_real"].round(2)
base_detalle["NETO"] = base_detalle["total_neto"].round(2)

base_detalle = base_detalle[
    [
        "FECHA",
        "SET_referencia",
        "CODIGO UNICO",
...
# SEPARAR PEN / USD
# ===============================
base_pen = base_detalle[base_detalle["MONEDA"] == "PEN"]
base_usd = base_detalle[base_detalle["MONEDA"] == "USD"]

c1, c2 = st.columns(2)

with c1:
    st.download_button(
        "📥 Descargar detalle PEN",
        exportar_csv(base_pen),
        "detalle_pen.csv",
        mime="text/csv"
    )
...
with c2:
    st.download_button(
        "📥 Descargar detalle USD",
        exportar_csv(base_usd),
        "detalle_usd.csv",
        mime="text/csv"
    )
