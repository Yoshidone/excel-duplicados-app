...
def leer_csv_seguro(f):
    """Lee CSV usando Polars directamente (sin conversión innecesaria a Pandas)."""
    ...
    for sep in [",", ";"]:
        try:
            ...
            df = pl.read_csv(
                ...
                separator=sep,
                ignore_errors=True,
                infer_schema_length=500,   # Reduce inferencia → más rápido
                low_memory=False,
                rechunk=True,
            )
            return df.to_pandas()
...
def leer_excel_seguro(f):
    """Lee Excel con pandas openpyxl, evita doble conversión innecesaria."""
    contenido = f.read() if hasattr(f, "read") else f
    return pd.read_excel(
        io.BytesIO(contenido),
        engine="openpyxl",
        dtype_backend="numpy_nullable",   # Reduce memoria vs object
    )
...
def cargar_archivo(file_bytes: bytes, nombre: str) -> pd.DataFrame:
    """
    Recibe bytes (serializables por st.cache_data) en vez del objeto file.
    Esto evita que Streamlit re-procese el archivo en cada interacción.
    """
    nombre = nombre.lower()
...
def normalizar_columnas(df: pd.DataFrame) -> pd.DataFrame:
    """Normaliza columnas y castea solo las columnas que realmente existen."""
    df.columns = df.columns.str.lower().str.strip()

    # Cast selectivo: solo columnas que existen
...
if archivos:
    ...
    for archivo in archivos:
        file_bytes = archivo.read()          # Lee una sola vez → bytes serializables
    ...
    # Libera memoria de los fragmentos
    del dfs
    gc.collect()
    ...
    if opcion_reporte:
        ...
        # Parse de fechas con cache implícito de pandas
        df["fecha"] = pd.to_datetime(
            df["x_create_date_gmt_peru"],
            errors="coerce",
            utc=False,
            format="mixed",   # pandas ≥2.0: infiere formato automáticamente
        )
        ...
        # Filtro eficiente con máscara booleana (evita query string overhead)
        ...
        if opcion_reporte in ["Comparación de comisiones", "Ambas"]:
            ...
            # Filtros vectorizados con str.startswith (evita apply/lambda)
            ...
            # Cálculos vectorizados en un solo bloque
            ...
            # Mostrar solo primeras 500 filas → evita renderizado lento
        ...
        if opcion_reporte in ["Reporte detallado", "Ambas"]:
            ...
            # Helper que devuelve serie vacía si columna no existe
            def col(name):
