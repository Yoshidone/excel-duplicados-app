...
with st.spinner("⏳ Procesando archivos..."):
    ...
    if errores:
        st.warning("⚠️ Archivos con error:\n" + "\n".join(errores))
...
st.success(f"✅ {len(df):,} filas cargadas · {len(df.columns)} columnas detectadas")
...
if not meses_disponibles:
    st.error("❌ No se encontraron fechas válidas en `x_create_date_gmt_peru`.")
    st.stop()
...
st.caption(f"📊 {len(df_filtrado):,} registros para {mes_sel} · {moneda_sel}")
...
if opcion_reporte in ["Comparación de comisiones", "Ambas"]:
    ...
    porcentaje = col_p.number_input(
        "Porcentaje comisión (%)",
        value=2.30, min_value=0.0, max_value=100.0, step=0.01,
    )
    fee_fijo = col_f.number_input(
        f"Fee fijo ({simbolo})",
        value=0.90, min_value=0.0, step=0.01,
    )

    pagos = df_filtrado[df_filtrado["tx_reference"].str.startswith("PY", na=False)].copy()
    fees  = df_filtrado[df_filtrado["tx_reference"].str.startswith("SF", na=False)].copy()
    ...
    if pagos.empty:
        ...
        if len(tabla) > 500:
            st.info(f"ℹ️ Mostrando 500 de {len(tabla):,} filas. Descarga el CSV para verlas todas.")

        st.dataframe(
            tabla.head(500),
            use_container_width=True,
            hide_index=True,
            height=altura_tabla(tabla.head(500)),
        )
        ...
        total_recaudo    = round(tabla["tx_amount_pago"].sum(),   2)
        total_base       = round(tabla["comision_base"].sum(),    2)
        ...
        col_m1.metric("💰 Recaudado",   f"{simbolo} {total_recaudo:,.2f}")
        col_m2.metric("💸 Comisiones",  f"{simbolo} {total_comisiones:,.2f}")
        col_m3.metric("🧾 Base",        f"{simbolo} {total_base:,.2f}")
        col_m4.metric("🏛 IGV (18%)",   f"{simbolo} {total_igv:,.2f}")

        col_m5, col_m6, col_m7, col_m8 = st.columns(4)
        col_m5.metric("📑 Total final",  f"{simbolo} {total_final:,.2f}")
        col_m6.metric("⚖️ Diferencia",   f"{simbolo} {total_diferencia:,.2f}")
        col_m7.metric("🔢 Operaciones",  f"{operaciones:,}")
        col_m8.metric("🧮 Neto",         f"{simbolo} {total_neto:,.2f}")
...
if opcion_reporte in ["Reporte detallado", "Ambas"]:
    ...
    pagos_det = df_filtrado[df_filtrado["tx_reference"].str.startswith("PY", na=False)].copy()
    fees_det  = df_filtrado[df_filtrado["tx_reference"].str.startswith("SF", na=False)].copy()

    pagos_det.rename(columns={"tx_amount": "RECAUDO",  "tx_reference": "PY_operation_no"}, inplace=True)
    fees_det.rename( columns={"tx_amount": "COMISION", "tx_reference": "SF_operation_no"}, inplace=True)

    detalle = pagos_det.merge(
        fees_det[["psp_tin", "COMISION", "SF_operation_no"]],
        on="psp_tin",
        how="left",
    )
    ...
    if len(reporte) > 500:
        st.info(f"ℹ️ Mostrando 500 de {len(reporte):,} filas. Descarga el CSV para verlas todas.")

    st.dataframe(
        reporte.head(500),
        use_container_width=True,
        hide_index=True,
        height=altura_tabla(reporte.head(500)),
    )
    ...
    col_d1.metric("💰 Recaudo total",   f"{simbolo} {total_recaudo_det:,.2f}")
    col_d2.metric("💸 Comisión total",  f"{simbolo} {total_comision_det:,.2f}")
    col_d3.metric("🔢 Operaciones",     f"{cantidad_operaciones_det:,}")
    col_d4.metric("🧾 Ticket promedio", f"{simbolo} {ticket_promedio:,.2f}")

    # ── CORRECCIÓN: neto en fila completa sin st.columns(1) ──────────────────
    st.metric("🧮 Neto", f"{simbolo} {neto_det:,.2f}")
    ...
    st.dataframe(
        ...
        use_container_width=True,
        hide_index=True,
        height=altura_tabla(resumen_comercios.head(500)),
    )
...
st.caption(f"📋 Total de registros en todos los meses: **{len(reporte_full):,}**")
